...
description: Promote a successful, repeatable approach into a reusable pattern note so future sessions don't re-derive the same thing. Trigger AGGRESSIVELY on any "this worked, save it for next time" intent - user does NOT need to say "pattern" or "extract" for this to activate. Intent triggers (English + Vietnamese, all should activate this skill) - Standardize / reuse intent: "this worked, let's keep doing it this way", "this approach is solid, standardize it", "next time we should do it this way", "make this our default approach", "let's adopt this", "cách này work, dùng tiếp đi", "lần sau làm thế này luôn cho khỏe", "approach này ngon, standardize đi", "default cho lần sau", "áp dụng cách này luôn"; Recurring success recognition: "we've done this 3 times now", "same approach worked again", "this is becoming a habit", "đã work mấy lần", "lần thứ N work rồi", "approach quen rồi", "lần nào cũng work"; Promote / codify language: "save as pattern", "extract as reusable", "codify this", "make it official", "promote to standard", "lưu thành pattern", "ghi nhớ cách này", "biến thành chuẩn", "đưa lên template"; Hand-off / institutional memory: "team should know about this", "share this approach with others", "team cần biết cách này", "phổ biến cho team". Trigger when a session ends with a novel approach worth reusing, when vault-session-end suggests promotion, or when the user mentions trying the same successful approach across 2+ contexts. Captures: when_to_use, steps, pitfalls, evidence (link sessions), anti-patterns, variations.
