...
description: Capture a non-trivial failure as a failure-log note (root cause + lesson + prevention recipe) so the same trap is not repeated. Trigger AGGRESSIVELY on any error/blocker/frustration signal - user does NOT need to say "log" or "failure" for this to activate. Intent triggers (English + Vietnamese, all should activate this skill) - Error / breakage signals: "it broke", "doesn't work", "this is broken", "stuck on X", "blocked by", "error keeps coming", "tool failed again", "fail rồi", "không chạy", "lỗi mãi không xong", "kẹt ở", "bị block", "tool fail lần thứ N rồi", "không work"; Frustration / repeated failure: "this keeps failing", "third time now", "ức chế quá", "frustrating", "lần thứ 3 rồi", "sao cứ lỗi mãi", "stuck again", "vẫn không được"; User correction / mistake flag: "you got that wrong", "that's not right", "you broke X", "đó sai rồi", "bạn nhầm rồi", "không phải thế", "làm hỏng rồi"; Model behaviour issue: "agent looped", "infinite loop", "Haiku refused", "over-cautious", "subagent timed out", "agent đang lặp", "vòng lặp vô tận", "Haiku từ chối"; Environment / tooling block: "permission denied", "MCP unavailable", "build failed", "test red", "deploy failed", "không có quyền", "build hỏng", "test fail", "deploy lỗi". Trigger immediately while the failure is fresh - even if the issue was eventually resolved, the resolution path itself is the value. Always include: failure mode, root cause (NOT symptom), lesson learned, prevention recipe for next time. BOUNDARY vs vault-capture: when the thing being saved IS a failure or breakdown of tooling, build, deploy, agent, MCP, or infrastructure (treo/hang, hỏng/broke, timed out, wiped, fail) - use THIS skill, NOT vault-capture, even when the user phrases it as a generic save intent like "ghi lại vụ này", "ghi nhận lại", "lưu lại bài học", "đáng ghi lại để khỏi quên" - the failure content decides, not the verb.
